...
#definition[有序对][如果 $x$ 和 $y$ 是两个对象，那么他们的_有序对 $(x,y)$ _也是一个对象，其中 $x$ 是有序对的第一个组分， $y$ 是有序对的第二个组分.
  $ (x, y) = (x', y') <==> x = x' and y = y' $
]
...
#tip-block[可以将加法构造为一个函数 $+:N times N -> N$ ，该函数被定义为 $(x, y) |-> x+y$ ]

#definition[有序$n$-元组和$n$-次笛卡尔积][
...
]<lemma:finite-selection>
#proof[证明 @lemma:finite-selection][
...
#proof[
  ...
  - $A times (B without C) = (A times B) without (A times C)$
    ...
    + $ & forall (x, y) in A times (B without C) \
      ...
      & => (x in A and y in B) and (x in A and y in.not C) $ 由于$x in A and y in B$，那么$x in A times B$，如果$(x, y) in A times C$，那么$x in A and y in C$，这与$y in.not C$矛盾，所以有$(x, y) in.not A times C$，进而$ & (x in A and y in B) and (x in A and y in.not C) \
...
#tip-block[这样的函数$h$被称为$f"和"g$的_直和（direct sum）_，并记作_$h = f plus.o g$_]
...
#problem[证明@lemma:cardinal-arithmetic]
#proof[
  ...
  + $X、Y$是有限集，那么$Y^X$是有限的，且 $\#(Y^X) = \#(Y)^(\#(X))$
    ...
    - 于是$Y^(X')$与$Y^X times Y$有相同的基数，由先前证明$\#(Y^X times Y) = \#(Y^X) times \#(Y)$，以及归纳假设$\#(Y^X) = b^a$，就有$\#(Y^X times Y) = b^a * b = b^(a+1)$，归纳结束
]
...
#problem[抽屉/鸽巢原理][令$A_1, ..., A_n$是有限集，且满足$\#(union.big_(i in {1,...,n})A_i) > n$，证明存在$i in {1,...,n}$使得$\#(A_i) >= 2$]
#proof[
