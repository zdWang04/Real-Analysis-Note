...
#lemma[有限序列是有界的]<lemma:finitie-seq-bounded>
#proof[证明@lemma:finitie-seq-bounded][
...
#lemma[柯西序列是有界的]<lemma:cauchy-is-bounded>
#proof[证明@lemma:cauchy-is-bounded][
...
#problem[证明@lemma:cauchy-is-bounded][
  见@lemma:cauchy-is-bounded 的证明过程，主要是分段考虑
]
...
#lemma[实数相等是良定义的][形式极限满足相等的自反、对称和传递性质]<lemma:well-definedness-eq-RR>
#proof[证明@lemma:well-definedness-eq-RR][
...
]<lemma:sum-of-cauchy-is-cauchy>
#proof[证明@lemma:sum-of-cauchy-is-cauchy][
...
]<lemma:well-definedness-add-cauchy>
#proof[证明@lemma:well-definedness-add-cauchy][
...
#note-block[
  @lemma:well-definedness-add-cauchy 其实就是证明实数加法是良定义的，完整的证明应当是$x = x' and y = y' => x+y = x'+y'$，这里只写了一部分，另外一部分在证明时使用$epsilon/2$不难得到
]
...
]<lemma:well-definedness-multi-RR>
#proof[证明@lemma:well-definedness-multi-RR][
...
#note-block[
  证明@lemma:well-definedness-multi-RR 时还应当证明$y = y' => x y = x y'$，这和@lemma:well-definedness-add-cauchy 的证明是一个道理
]
...
#remark[
  令$x = "LIM"_(n->infinity)a_n, y = "LIM"_(n->infinity)b_n$，类似的，根据实数相反数的定义以及乘法和加法就有$ x - y = x + (-y) = "LIM"_(n->infinity)a_n + "LIM"_(n->infinity)-b_n = "LIM"_(n->infinity)(a_n - b_n) $
]
...
#lemma[$x in RR and x != 0$，那么存在远离$0$的柯西序列$(a_n)_(n=1)^infinity$，满足$x = "LIM"_(n->infinity)a_n$]<lemma:non-zero-RR-eq-seq-away-from-zero>
#proof[证明@lemma:non-zero-RR-eq-seq-away-from-zero][
...
#remark[
  @lemma:non-zero-RR-eq-seq-away-from-zero 说明每个非零实数总与某个远离0的柯西序列对应
]

#lemma[如果一个柯西序列$(a_n)_(n=1)^infinity$是远离$0$的，那么序列$(a_n^(-1))_(n=1)^infinity$也是柯西序列]<lemma:x-and-x-reciprocal-both-away-from-zero>
#proof[证明@lemma:x-and-x-reciprocal-both-away-from-zero][
...
#remark[
  + @lemma:non-zero-RR-eq-seq-away-from-zero 保证了一个柯西序列是远离0的，那么它的形式极限就能和一个非0的实数对应
  + @lemma:x-and-x-reciprocal-both-away-from-zero 保证了远离0的柯西序列的“倒数序列”也是柯西的，也即“倒数序列”的形式极限也是实数中的一个元素，保证了倒数运算对非0实数的封闭性
...
]<lemma:well-definedness-RR-reciprocal>
#proof[证明@lemma:well-definedness-RR-reciprocal][
...
#problem[证明@lemma:well-definedness-eq-RR][见@lemma:well-definedness-eq-RR 的证明]
#problem[证明@lemma:well-definedness-multi-RR][见@lemma:well-definedness-multi-RR 的证明]
#problem[$a,b in QQ$，证明$a = b <=> "LIM"_(n->infinity)a = "LIM"_(n->infinity)b$]
#proof[
  + $=>$

    由于$a = b => a-b = 0 => abs(a-b) = 0$
...
]<def:rational-seq-bounded-away-from-zero>

#note-block[
  由于实数被定义为了柯西序列的形式极限，所以想要对实数的正负下定义，先要对序列的正负下定义，@def:rational-seq-bounded-away-from-zero 正是这个目的
]
...
#lemma[实数$x$是负的，当且仅当$-x$是正的]<lem:negation-of-negative-is-positive>
#proof[证明@lem:negation-of-negative-is-positive][
...
#lemma[实数$x,y$都是正的，那么$x+y$和$x y$也是正的]<lem:positivity-of-sum-and-product>
#proof[证明@lem:positivity-of-sum-and-product][
...
]<prop:real-order-basic-properties>
#proof[证明@prop:real-order-basic-properties][

  令$x = "LIM"_(n->infinity)a_n,y = "LIM"_(n->infinity)b_n,z = "LIM"_(n->infinity)c_n$
  + 见@thm:real-trichotomy 的证明
  + $x > y <=> y < x$
    - $=>$
...
#lemma[正实数的倒数也是正实数][$x in RR^+ => x^(-1) in RR^+$]<lem:positivity-of-inverse>
#proof[证明@lem:positivity-of-inverse][
  $x$是正实数，且$x x^(-1)=1$，易知$x^(-1)!=0$；如果$x^(-1)$是负的，那么$x x^(-1)$也是负的，矛盾，由实数的三歧性，$x^(-1)$是正的
]

#lemma[正实数的倒数逆转序][
  $x, y in RR^+, x > y => x^(-1) < y^(-1)$
]<lem:inverse-reverses-order>
#proof[证明@lem:inverse-reverses-order][
  由于$y$是正的，那么$x > y=> x$也是正的，假设$x^(-1) >= y^(-1)$，就有$ 1 = x x^(-1) > y x^(-1) >= y y^(-1) = 1 $于是$1 > 1$，矛盾，由实数的序的三歧性，$x^(-1) < y^(-1)$
]

#lemma[非负柯西序列的形式极限也是非负的][$(a_n)_(n=1)^infinity$是非负有理数组成的柯西序列，那么实数$x = "LIM"_(n->infinity)a_n$是非负实数]<lem:limit-of-nonneg-cauchy-is-nonneg>
#proof[证明@lem:limit-of-nonneg-cauchy-is-nonneg][
...
]<cor:order-preservation-of-limits>
#proof[证明@cor:order-preservation-of-limits][
...
#remark[
  @lem:limit-of-nonneg-cauchy-is-nonneg 和@cor:order-preservation-of-limits 表明，非负实数集合是闭的
]

#lemma[用有理数界定实数][
  $forall x in RR^+, exists q in QQ^+,N in ZZ^+, q<=x<=N$
]<lem:rational-bounds-for-positive-real>
#proof[证明@lem:rational-bounds-for-positive-real][
...
]<cor:archimedean-property>
#proof[证明@cor:archimedean-property][
...
]<prop:max-min-operation-properties>


#problem[证明@thm:real-trichotomy 和 @prop:real-order-basic-properties][
  见@thm:real-trichotomy 和@prop:real-order-basic-properties 的证明
]
#problem[证明实数的序也满足有理数的序的性质][
  见@prop:real-order-basic-properties 的证明
]
...
#remark[
  ...
  + 上面证明了这样的$N$存在且唯一，所以可以将向上/向下取整也迁移到实数上
]
...
#remark[
  无论多么小的一个正实数，总能找到一个超大的正整数，使得该正整数的倒数比超小正实数还小，但还是比零大
]
...
#problem[$x, y in RR, epsilon in RR^+$. 证明：(1) $abs(x - y) < epsilon <=>-epsilon + y< x < epsilon+y$. (2) $abs(x - y) <= epsilon <=> -epsilon + y<= x <= epsilon + y$]
#proof[
...
#problem[证明：(1) $forall x, y in RR, forall epsilon in RR^+, x <= y + epsilon <=> x <= y$. (2) $forall epsilon in RR^+, abs(x - y) <= epsilon <=> x = y$]
...
#proof[
  ...
  #note-block[
    ...
    + 关于$abs(a_i - y) <= epsilon$这个表述，这个看起来好像是在进行柯西序列和一个常数列等价的判定，但是$y$有可能不是有理数，而我们现在的柯西序列都是定义在有理数序列上，所以感觉上这么写是有问题的。_但是_，这个式子的意思是在对一个有理数与实数之差的绝对值进行不等式比较，而实数的绝对值的性质已经进行了证明，所以是没问题的。
  ]
]

#problem[证明@prop:max-min-operation-properties]
#proof[
  + $max(x, y) = -min(-x, -y)$
...
#pagebreak()
#lemma[最小上界的唯一性]<lem:uniqueness-of-supremum>
#proof[证明@lem:uniqueness-of-supremum][
...
]<thm:existence-of-supremum>
#proof[证明@thm:existence-of-supremum][
...
#remark[
  + 上确界的良定义由@thm:existence-of-supremum 和@lem:uniqueness-of-supremum 所保障
...
#property[实数的整数次幂运算也满足有理数的指数运算性质][
  ...
  + $abs(x^n) = abs(x)^n$
]<prop:integer-power-operation-properties>
#proof[元证明@prop:integer-power-operation-properties][
...
]<lem:existence-of-nth-root-for-non-neg-real>
#proof[
...
#property[非负实数的$n$次方根的性质][
  ...
  + $(x y)^(1/n) = x^(1/n) y^(1/n)$
  + $(x^(1/n))""^(1/m) = x^(1/(n m))$
]<prop:nth-root-properties-for-non-neg-real>
...
]<lemma:well-definedness-rational-power>
#proof[
...
#property[正实数有理数次幂的性质][
  ...
  + $x < 1, x^q > x^r <=> q < r$
  + $(x y)^q = x^q y^q$
]<prop:positive-rational-power-properties-for-real>

#lemma[二项式展开引理][
  $forall a, b in RR_(>=0), n in ZZ^+, (a+b)^n >= a^n + n a^(n-1)b$
]<lem:binomial-expansion>
#proof[
  对$n$进行数学归纳
...
#theorem[差幂公式][
  $forall a, b in RR, n in ZZ^+$，有下式成立$ a^n - b^n = (a-b)(a^(n-1) + a^(n-2)b + ... + a^(n-j)b^(j-1) + ... + a b^(n-2) + b^(n-1) ) $]<thm:difference-of-powers-formula>
#proof[
...
]<lem:monotonicity-of-positive-integer-power>
#proof[
...
]<lem:local-lipschitz-estimate-for-power-function>
#proof[
  + 减量上界估计\
    由@thm:difference-of-powers-formula 就有$ & "   "y^n - (y - epsilon)^(n) \
  ...
  + 增量下界估计\
    由于$y in RR_(>=0), epsilon in RR^+$，那么可以使用@lem:binomial-expansion，就有$ (y + epsilon)^n >= y^n + n y^(n-1) epsilon => (y+epsilon)^n - y^n >= n y^(n-1)epsilon $
]


#corollary[幂函数的局部保号性][
  $forall y, delta in RR^+, n in ZZ^+, y^n > delta => exists epsilon in RR^+, (y-epsilon)^n > delta$
]<cor:local-sign-preservation-of-power-function>
#proof[
...
]<prop:max-min-power-properties>

#problem[证明@prop:nth-root-properties-for-non-neg-real]
...
#problem[证明@prop:positive-rational-power-properties-for-real]
#proof[
...
#problem[证明@prop:max-min-power-properties]
#proof[
  + $max(x^q, y^q) = max(x, y)^q$\
